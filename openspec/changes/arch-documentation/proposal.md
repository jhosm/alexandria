...
LLM agents using Alexandria need to understand the architecture — the patterns for building, exposing, and consuming APIs — not just individual API specs. Today, Alexandria's registry only supports API entries (with a required OpenAPI spec). There's no way to index standalone documentation like architectural patterns, and no dedicated MCP tool to guide the agent toward using that documentation in the right contexts (writing API code, understanding integration patterns, etc.).
