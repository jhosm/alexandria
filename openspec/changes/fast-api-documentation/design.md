...
Alexandria indexes API documentation (OpenAPI specs + markdown) into SQLite with hybrid search, served via MCP over stdio. The registry (`apis.yml`) currently only supports API entries with a required `spec` field. The MCP server exposes three tools: `list-apis`, `search-docs`, `get-api-endpoints`. We need to support standalone documentation collections (like FAST) alongside APIs, with a dedicated MCP tool for FAST and a clearer name for the existing search tool.
...
- Add `search-fast-docs` tool with a description that guides agents to use it for FAST architecture contexts

**Non-Goals:**
- Changing the database schema (docs entries use the existing `apis` + `chunks` tables)
- Adding new chunk types (FAST docs produce the same `glossary`/`use-case`/`guide` types)
- Generic "search any doc collection" tool (FAST gets a dedicated tool; other doc collections can be added later following the same pattern)
...
### D4: `search-fast-docs` hardcoded to the "fast" docs entry

**Choice**: The `search-fast-docs` tool internally resolves the "fast" entry by name and scopes all queries to that `apiId`. It accepts `query` and optional `types` parameters (no `apiName` — it's always "fast").
...
**Rationale**: The tool's value is in its description, not its implementation. A rich description like "Search FAST architecture documentation. Use this when you need to understand FAST concepts, write code to expose an API, or write code to consume an API" guides agents to the right tool at the right time. The implementation is just `searchHybrid` with a fixed `apiId`.
...
**Rationale**: With two search tools, the names and descriptions must clearly partition the search space. `search-api-docs` is for API-specific content; `search-fast-docs` is for FAST architecture content.
...
- **"fast" entry must exist** → `search-fast-docs` will return an error if "fast" hasn't been ingested. Mitigation: clear error message ("FAST documentation not indexed. Run ingestion first."). Same pattern as `get-api-endpoints` returning "API not found."
